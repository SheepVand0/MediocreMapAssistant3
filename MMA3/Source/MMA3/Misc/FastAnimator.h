...
public:
	...
	FVectorAnimationKey(float time, FVector value) {
		Time = time;
		Value = value;
	}
...
public:
	...
	FVectorAnimation(TArray<FVectorAnimationKey*> keys, SActionPtrBase<void,float>* callback, SActionPtrBase<void, float>* finishedCallback = nullptr) {
		Keys = keys;
		Callback = TSharedPtr<SActionPtrBase<void, float>>(callback);
		FinishedCallback = TSharedPtr<SActionPtrBase<void, float>>(finishedCallback);
	}

	TArray<FVectorAnimationKey*> Keys;
	TSharedPtr<SActionPtrBase<void, float>> Callback;
	TSharedPtr<SActionPtrBase<void, float>> FinishedCallback;

	FVector Evaluate(float time);

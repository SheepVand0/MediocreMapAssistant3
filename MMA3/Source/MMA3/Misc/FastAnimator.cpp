...
float FFastAnimation::Evaluate(float time, bool& finished) {
	...
	while (l_i < Keys.Num() - 1) {
		...
		if (l_Key->Time <= time && l_NextKey->Time >= time) {
			...
			float l_DiffDuration = time - l_Key->Time;
			float l_Value = l_Key->Value + ((l_DiffDuration / (l_NextKey->Time - l_Key->Time)) * (l_NextKey->Value - l_Key->Value));
			return l_Value;
		}
		l_i += 1;
	}
...
void AFastAnimator::ProcessAnimation(FFastAnimation* animation, float time) {
	UE_LOG(LogTemp, Warning, TEXT("Processing Animation: %s"), *(animation->AnimationId));
...
FVector FVectorAnimation::Evaluate(float time)
{
	int l_i = 0;
	while (l_i < Keys.Num() - 1) {
		auto l_Key = Keys[l_i];
		auto l_NextKey = Keys[l_i + 1];

		if (l_Key->Time <= time && l_NextKey->Time >= time) {
			float l_Duration = l_NextKey->Time - l_Key->Time;
			float l_Perc = (time - l_Key->Time) / l_Duration;
			float l_Mutl = l_Perc;
			if (l_Key->ExponentFunction != nullptr) {
				l_Mutl *= l_Key->ExponentFunction->Invoke(l_Perc);
			}

			FVector l_Value = l_Key->Value + ((l_NextKey->Value - l_Key->Value) * l_Mutl);
			return l_Value;
		}
		l_i++;
	}

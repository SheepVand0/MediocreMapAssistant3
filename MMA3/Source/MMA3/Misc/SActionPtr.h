...
class SMethodPtr : public SActionPtrBase<t_returnType, t_parameters...> {
...
template<class t_returnType, class... t_parameters>
class SActionPtr : public SActionPtrBase<t_returnType, t_parameters...> {
...
public:
    ...
    TArray<SMethodPtr<UObject, void, t_parameters...>> Delegates;

    void Bind(SMethodPtr<UObject, void, t_parameters...> target) {
        Delegates.Add(target);
    }

    void Broadcast(t_parameters... params) {
        for (int x = 0; x < Delegates.Num(); x++) {
            Delegates[x].Invoke(params...);
        }
    }

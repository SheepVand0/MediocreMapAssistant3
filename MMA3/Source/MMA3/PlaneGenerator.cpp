...
void APlaneGenerator::OnConstruction(const FTransform& transform)
{
	...
	if (Wall.IsNull()) {
		Wall = GetWorld()->SpawnActor<ACWall>();
		Wall->SetActorLocation(GetActorLocation());
	}

	FWallData l_TestWallData;
	l_TestWallData.Beat = 0;
	l_TestWallData.Duration = 2;
	l_TestWallData.Height = 3;
	l_TestWallData.Line = 0;
	l_TestWallData.Width = 1;
	l_TestWallData.Layer = 0;

	Wall->SetData(l_TestWallData);
	Wall->SetExtremumsBeats(0, 100);
	Wall->BuildMesh();*/
	...
	TArray<FProcMeshTangent> l_Tangent;
	UKismetProceduralMeshLibrary::GenerateBoxMesh(FVector(100, 200, 800), l_Vertices, l_Triangles, l_Normals, l_UVs, l_Tangent);

	PlaneMesh->CreateMeshSection(0, l_Vertices, l_Triangles, l_Normals, l_UVs, TArray<FColor>(), TArray<FProcMeshTangent>(), true);
	...
	/*TArray<FVector> l_Vert;
	TArray<FLinearColor> l_Color;
	URenderSoundVisualization::GenerateVisMesh(PlaneMesh, 10, 10, 10, 10, l_Vert, l_Color);*/
}
